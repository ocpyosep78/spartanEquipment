...
class ACCOUNTING_QUICKBOOKS_ENTITIES_SALESRECEIPT extends ACCOUNTING_QUICKBOOKS_ENTITIES_BASE
{
	protected function xml()
	{
		...
		if (isc_strtolower($this->data->service) == 'salesreceiptmod') {
			$reference = $this->quickbooks->getAccountingReference($this->data->nodeId, 'salesreceipt');
		...
		if (isset($order['products']) && is_array($order['products'])) {
			foreach ($order['products'] as $product) {
				...
				if (isc_strtolower($this->data->service) == 'salesreceiptmod') {
					$this->xmlNode->startElement('SalesReceiptLineMod');
					...
					$this->xmlNode->startElement('SalesReceiptLineAdd');
				}
				...
				$this->xmlNode->writeElement('Amount', number_format($product['prodorderamount'] * $product['prodorderquantity'], 2, '.', ''));
