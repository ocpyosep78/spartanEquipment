...
class ACCOUNTING_QUICKBOOKS_SERVICES_SALESRECEIPTADD extends ACCOUNTING_QUICKBOOKS_SERVICES_BASE
{
	/**
	 * Execute the service
	 *
	...
	protected function response()
	{
		...
		if (($code = parent::handleResponse()) !== true) {
			throw new Exception('An error occured when trying to handle the response from the SalesReceiptAdd spool ' . $this->spoolId);
		}
		...
		if ($txnid !== '' && $sequence !== '') {
			...
			if ($this->quickbooks->compareClientVersion('4.0')) {
				...
				foreach ($this->spoolData['accountingspoolnode']['products'] as $key => $product) {
					...
					if (!$reference) {
						throw new Exception('Cannot find the product account reference data (Prod# ' . $product['productid'] . ') for the SalesReceiptAdd spool ' . $this->spoolId);
					}
				...
				if (empty($products)) {
					throw new Exception('Cannot load any products for the "InventoryAdjustmentAdd" child service for the SalesReceiptAdd spool ' . $this->spoolId);
				}
				...
				throw new Exception('Cannot initiate an "InventoryAdjustmentAdd" child service for the SalesReceiptAdd spool ' . $this->spoolId);
			}

			return true;
		}

		throw new Exception('Cannot insert the order using the information in the SalesReceiptAdd spool ' . $this->spoolId);
	}
