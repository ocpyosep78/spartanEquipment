...
class ACCOUNTING_QUICKBOOKS_SERVICES_SALESRECEIPTMOD extends ACCOUNTING_QUICKBOOKS_SERVICES_BASE
{
	/**
	 * Execute the service
	 *
	...
	protected function run()
	{
		...
		if (!$this->isChildServiceInSpool('salesorderadd', $children) && !$this->getAccountingReference($this->spoolData['accountingspoolnodeid'], 'order')) {
			...
			if (isId($addSpoolId)) {
				if ($this->execSibling($serviceOutput, 'salesorderadd', 'run', array('spoolId' => $addSpoolId))) {
					...
					throw new Exception('Failed in running the child service SalesReceiptAdd for the SalesReceiptMod spool ' . $this->spoolId);
				}
			}

			throw new Exception('Cannot initialise new child service "SalesReceiptAdd" for SalesReceiptMod spool ' . $this->spoolId);
		}
		...
		if (!isId($customerId)) {
			throw new Exception('Cannot find the customer ID ' . $customerId . ' for SalesReceiptAdd spool ' . $this->spoolId);
		}
		...
		if (!$this->isChildServiceInSpool('customeradd') && !$this->getAccountingReference($customerId, 'customer')) {
			...
			if (isId($customerSpoolId)) {
				if ($this->execSibling($serviceOutput, 'customeradd', 'run', array('spoolId' => $customerSpoolId))) {
					...
					throw new Exception('Failed in running the child service CustomerAdd for the parent SalesReceiptAdd spool ' . $this->spoolId);
				}
			}

			throw new Exception('Cannot initialise new child service "CustomerAdd" for SalesReceiptAdd spool ' . $this->spoolId);
		}
		...
		if ($productKey !== false) {
			...
			if (isId($productSpoolId)) {
				if ($this->execSibling($serviceOutput, 'iteminventoryadd', 'run', array('spoolId' => $productSpoolId))) {
					...
					throw new Exception('Failed in running the child service ItemInventoryAdd for the parent SalesReceiptAdd spool ' . $this->spoolId);
				}
			}

			throw new Exception('Cannot initialise new child service "ItemInventoryAdd" for SalesReceiptAdd spool ' . $this->spoolId);
		}
	...
	protected function response()
	{
		...
		if (($code = parent::handleResponse()) !== true) {
			throw new Exception('An error occured when trying to handle the response from the SalesReceiptEdit spool ' . $this->spoolId);
		}

		/**
		 * SalesReceipt was successfully added, now all we need to do is to store the association information for this order and decerement the stock on hand levels
		 */
		$txnid		= trim(@(string)$this->data->info->SalesReceiptRet->TxnID);
		$sequence	= trim(@(string)$this->data->info->SalesReceiptRet->EditSequence);
		...
		if ($txnid !== '' && $sequence !== '') {
			...
			$this->quickbooks->setAccountingReference($this->spoolData['accountingspoolnodeid'], 'salesreceipt', $reference);

			/**
			 * Next we need to save the order - product association id that QuickBooks use
			 */
			foreach ($this->data->info->SalesReceiptRet->SalesReceiptLineRet as $lineItem) {
				$productId = null;
			...
			if ($this->quickbooks->compareClientVersion('4.0')) {
				...
				if (isset($this->spoolData['accountingspoolnode']['previous'])) {
					foreach ($this->spoolData['accountingspoolnode']['previous']['products'] as $key => $product) {
						...
						if (!$reference) {
							throw new Exception('Cannot find the product account reference data (Prod# ' . $product['productid'] . ') for the SalesReceiptEdit spool ' . $this->spoolId);
						}
				...
				if (empty($products)) {
					throw new Exception('Cannot load any products for the "InventoryAdjustmentAdd" child service for the SalesReceiptEdit spool ' . $this->spoolId);
				}
				...
				throw new Exception('Cannot initiate an "InventoryAdjustmentAdd" child service for the SalesReceiptEdit spool ' . $this->spoolId);
			}

			return true;
		}

		throw new Exception('Cannot insert the order using the information in the SalesReceiptEdit spool ' . $this->spoolId);
	}
