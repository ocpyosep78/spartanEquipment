...
class ACCOUNTING_QUICKBOOKS_SESSION
{
	 ...
	 * @access public
	 */
	public function setQBSessSession()
	{
	...
	/**
	 * Compare the qbXML version and country code to the client's qbXML version and country code
	 *
	 * Method is a wrapper function for compareClientVersion() and compareClientCountry() and will return TRUE if both function
	 * return TRUE
	 *
	 * @access protected
	 * @param string $version The version to compare
	 * @param string $country The country code to compare against
	 * @return bool TRUE if the qbXML version is lower than the client's version and if the country codes match, FALSE otherwise
	 */
	public function compareClientQBXML($version, $country)
	{
		if ($version == '' || $country == '') {
			return false;
		}

		return (bool)($this->compareClientVersion($version) && $this->compareClientCountry($country));
	}
	...
	public function compareClientVersion($version)
	{
		...
		$this->getQBSession('QBXML_VERSION', $clientVersion);

		$clientVersion = (string)$clientVersion;

		if ($clientVersion == '' || version_compare($clientVersion, $version) == 1) {
			return false;
		}
	...
	public function compareClientCountry($country)
	{
		...
		$this->getQBSession('CLIENT_COUNTRY', $clientCountry);

		if ($clientCountry == '' || strtolower($country) !== strtolower($clientCountry)) {
			return false;
		}
