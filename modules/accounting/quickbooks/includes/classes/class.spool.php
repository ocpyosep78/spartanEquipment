...
class ACCOUNTING_QUICKBOOKS_SPOOL
{
	...
	public function getJobDataProperty(&$reference, $jobid, $property)
	{
		...
		/**
		 * Apply the filter here if we are requesting the xml
		 */
		if ($property == 'xml') {
			$reference = $this->filterSpoolFileXMLString($reference);
		}
	 ...
	 *
	 * Method will get the XML of the spooled job
	 *
	 * @access public
	...
	/**
	 * Filter our any content in the XML string that is not available with the current client connection
	 *
	 * Method will filter out (remove) any XML content that is not available with the current client connection based on an XML string
	 *
	 * @access protected
	 * @param string $xml The XML string to parse
	 * @param string $serviceName The optional type of service this XML string represents. Default will look for it
	 * @return string The parsed XML string on success, FALSE on failure
	 */
	private function filterSpoolFileXMLString($xml, $serviceName='')
	{
		if ($xml == '') {
			return false;
		}
		...
		$obj = $this->filterSpoolFileXMLObject($obj, $service);
	...
	/**
	 * Filter our any content in the SimpleXML object that is not available with the current client connection
	 *
	 * Method will filter out (remove) any XML content that is not available with the current client connection based on a SimpleXML object
	 *
	 * @access protected
	 * @param object &$obj The referrenced SimpleXMLElement to filter
	 * @param string $serviceName The optional type of service this SimpleXML object represents. Default will look for it
	 * @return object The parse object on success, FALSE on failure
	 */
	private function filterSpoolFileXMLObject($obj, $serviceName='')
	{
		if (!is_object($obj)) {
			return false;
		}
		...
		/**
		 * Look for the service if we have to
		 */
		if ($serviceName == '') {
			$tmp = $obj->xpath('//QBXMLMsgsRq/*[1]');

			if (empty($tmp) || !isset($tmp[0])) {
				return false;
			}

			$serviceName = $tmp[0]->getName();
			$serviceNameRq = $serviceName;
		...
		/**
		 * We need to go to the actual source of the information so we can unset it, which is the only way of removing it
		 */
		$tmpObj =& $obj->QBXMLMsgsRq->$serviceNameRq->$serviceName;

		$children = array();
		foreach ($tmpObj->children() as $key => $child) {
			$children[$key] = $child;
		}

		switch (strtolower($serviceName)) {
			case 'customeradd':
				if ($this->session->compareClientVersion('4.0') && array_key_exists("PriceLevelRef", $children)) {
					unset($tmpObj->PriceLevelRef);
				}

				if ($this->session->compareClientVersion('3.0') && array_key_exists("IsActive", $children)) {
					unset($tmpObj->IsActive);
				}
			...
			case 'iteminventoryadd':
				if ($this->session->compareClientVersion('7.0') && array_key_exists("ManufacturerPartNumber", $children)) {
					unset($tmpObj->ManufacturerPartNumber);
				}
