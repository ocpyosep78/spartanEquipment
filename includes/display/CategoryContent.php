<?php
	...
	CLASS ISC_CATEGORYCONTENT_PANEL extends PRODUCTS_PANEL
	{
		public function SetPanelSettings()
		{
            ...
            //mz "the zebra" to fix the compare issue
            if ($GLOBALS['ISC_CLASS_CATEGORY']->GetNumProducts() > 1) {
                ...
                $GLOBALS['ndxCompareButton'] = "<p><input type='submit' class='compareBtn' value='' onclick=\"";
                $GLOBALS['ndxCompareButton'] .= "return CheckForm('compare');\"";
                $GLOBALS['ndxCompareButton'] .= "style='float: right;display:'><br /></p>";
            }
			...
			foreach($products as $row) {
				...
				$GLOBALS['LeasePrice'] = trim($GLOBALS['ProductPrice'], "$"); 
				$product_data = new ISC_PRODUCT($GLOBALS['ProductId']);
				if ($product_data->hasVideos()) {
					$GLOBALS['AndVideo'] = ' <em>&amp; Videos</em>';
				} else {
					$GLOBALS['AndVideo'] = '';
				}
				// for list style
