...
class ISC_CHECKOUT_PROVIDER extends ISC_MODULE
{
	...
	*/
	...
	/**
	* Filters and returns only loggable information for a credit card (typically the last 4 digits with others replaced
	* with a placeholder)
	*
	* @param string $creditCardNumber card number, which is assumed to be valid and has non-numeric characters removed
	* @return string
	*/
	protected function _filterCreditCardNumber ($number)
	{
		// generic filter statement which replaces the entire card number except the last four digits with placeholder
		// characters - since we want to log exactly what is being sent to a gateway, it's intended to work even if the
		// payment gateway is sending spaces/dashes

		// note: this is complex, and can be simplified if we implement a strict policy of trimming cc numbers, but, we
		// don't right now and I'd rather not implement without fully testing each gateway (i.e., this is easier even
		// if it looks ridiculous)

		$lastN	= 4; // how many digits to find

		// find the last n digits by normalising the provided string to numbers only
		$last	= preg_replace('/[^\\d]/', '', $number); // the original cc no reduced to numbers only
		$last	= isc_substr($last, isc_strlen($last) - $lastN); // the last digits
		$last	= str_split($last); // make it an array because it's easier to work with next...

		// using the original string, create a replacement with all numbers replaced with placeholder character
		$replacement = preg_replace('/\\d/', '#', $number);
		while (!empty($last)) {
			// assuming the original input did not include placeholders itself... replace the last occurrence of the
			// placeholder with the last digit, run it until we have no more 'last' digits
			$replacement = preg_replace('/#([^#]*)$/', array_pop($last) . '\\1', $replacement, 1);
		}
	...
	/**
	* Filters and returns only loggable information for a credit card security code. That is, every number is replaced
	* with a placeholder.
	*
	* @param string $number
	*/
	protected function _filterCardSecurityCode ($number)
	{
		return preg_replace('/\\d/', '#', $number);
	}

	/**
	* Generic logging function that takes the provided $logDetails and logs it against this payment module with the
	* current order id.
	*
	* @param string $logDetails request data being sent to payment gateway but make sure cardholder data is filtered first
	*/
	protected function _logGatewayRequest ($logDetails)
	{
		return $GLOBALS['ISC_CLASS_LOG']->LogSystemDebug(
			array(
				'payment', $this->GetName()
			),
			GetLang('PaymentGatewayRequestDebug', array(
				'order' => $this->GetCombinedOrderId(),
			)),
			nl2br(isc_html_escape($logDetails))
		);
	}
