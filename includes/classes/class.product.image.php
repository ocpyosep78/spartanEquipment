...
class ISC_PRODUCT_IMAGE {
	...
	*/
	 ...
	 *
	 * @param string image path relative to product_images
	...
	*/
	public function removeSourceFile()
	{
		...
		if (file_exists($filePath) && is_file($filePath)) {
			...
			$GLOBALS['ISC_CLASS_LOG']->LogSystemDebug('general', 'ISC_PRODUCT_IMAGE::removeSourceFile has deleted ' . $filePath . ' (image id: ' . $this->getProductImageId() . ') for product ' . $this->getProductId() . '#' . $this->getProductHash(), trace() . "<br/>" . var_export($this, true));
		}
	...
	*/
	...
	{
		$filePath = $this->getAbsoluteResizedFilePath($size, false, false);
		if ($filePath && file_exists($filePath) && is_file($filePath)) {
			if (!@unlink($filePath)) {
				$error = error_get_last();
				throw new ISC_PRODUCT_IMAGE_CANNOTDELETEFILE_EXCEPTION(GetLang('ProductImageDeleteResizedFileError'));
			}
			$GLOBALS['ISC_CLASS_LOG']->LogSystemDebug('general', 'ISC_PRODUCT_IMAGE::removeResizedFile has deleted ' . $filePath . ' for product ' . $this->getProductId() . '#' . $this->getProductHash() . ' size ' . $size, trace() . "<br/>" . var_export($this, true));
		}
