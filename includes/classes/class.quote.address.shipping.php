...
class ISC_QUOTE_ADDRESS_SHIPPING extends ISC_QUOTE_ADDRESS
{
	...
	*/
	...
	public function getShippingMethodQuotes($method, $enableRealTime=true)
	{
		...
		$itemPriceIncludeTax = (bool)$shippingModule->GetValue('includetax');
		...
		foreach ($shippingQuoteItems as $k => $item) {
			if (!$itemPriceIncludeTax) {
				...
				$price = $item->getPrice($itemPriceIncludeTax);
			}
	...
	public function getAvailableShippingMethods($enableRealTime = true, $cacheResults = true)
	{
		...
		// To make sure that we have at least one valid shipping method set.
		$query = "
			SELECT *
		...
		$shippingMethodNum = $GLOBALS['ISC_CLASS_DB']->CountResult($result);

		// Free shipping method
		if ($this->allowsFreeShipping() && $shippingMethodNum) {
			$freeShippingQuotes = array(
			...
			$shippingQuotes[] = $freeShippingQuotes;
			if (empty($shippingQuoteItems)) {
				return array($freeShippingQuotes);
			}
		}
