...
class ISC_LINKER {
	...
	private function GetCategories()
	{
		...
		foreach($categories as $catid => $catname) {
			...
			echo sprintf('<result title="%s" icon="images/category.gif" catid="%s" %s><![CDATA[%s]]></result>', isc_html_escape($catname), $catid, $catpadding, $catlink);
		}
		echo '</results>';
	}
	...
	private function GetBrands()
	{
		...
		while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($result)) {
			echo sprintf('<result title="%s" icon="images/brand.gif"><![CDATA[%s]]></result>', isc_html_escape(isc_html_escape($row['brandname'])), BrandLink($row['brandname']));
			$hasBrands = true;
		}
		if(!$hasBrands) {
	...
	private function GetPages()
	{
		...
		if(!count($pages)) {
			...
			foreach ($pages as $page) {
				...
				echo sprintf('<result title="%s" icon="images/page.gif" padding="' . (($page['depth'] * 18) + 6) . '"><![CDATA[%s]]></result>', isc_html_escape(isc_html_escape($page['pagetitle'])), $pageLink);
			}
		}
		...
		if(!isset($_REQUEST['searchQuery']) && !isset($_REQUEST['category']) || (isset($_REQUEST['searchQuery']) && isc_strlen($_REQUEST['searchQuery']) <= 3)) {
			...
			if($ResultCount == 0) {
				...
				while($product = $GLOBALS['ISC_CLASS_DB']->Fetch($products)) {
					echo sprintf('<result title="%s" icon="images/product.gif"><![CDATA[%s]]></result>', isc_html_escape(isc_html_escape($product['prodname'])), ProdLink($product['prodname']));
				}
			}
		}
		echo '</results>';
