	...
	*/
	...
	public function HandleToDo($do)
	{
		...
		// if we need to update the cache for this action
			...
			// verify the details against eBay before we get the cache file from eBay.
			try {
				ISC_ADMIN_EBAY_OPERATIONS::geteBayOfficialTime();
			}
			catch (ISC_EBAY_API_EXCEPTION $ex) {
				FlashMessage(GetLang('EbaySettingsNotValid'), MSG_ERROR, '', 'EbayConfig');
				$GLOBALS['ISC_NEW_CFG']['EbaySettingsValid'] = false;
				$this->ManageEbay(false);
				exit;
			}
	...
	public function ManageEbayTemplateGrid(&$numTemplate)
	{
		...
		if($numTemplate > 0) {
			...
			while ($row = $GLOBALS["ISC_CLASS_DB"]->Fetch($templateResult))
			{
				...
				if($this->auth->HasPermission(AUTH_Ebay_Selling)) {
					if ($row['enabled'] == 1) {
