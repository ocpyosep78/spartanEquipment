<?php
	...
	class ISC_ADMIN_REMOTE_CUSTOMERS extends ISC_ADMIN_REMOTE_BASE
	{
		...
		public function HandleToDo()
		{
			...
			switch ($what) {
				case "getselectedstates":
					if ($this->auth->HasPermission(AUTH_Edit_Customers) || $this->auth->HasPermission(AUTH_Add_Customer)) {
		...
		{
			if(!isset($_REQUEST['customerId']) || !$this->auth->HasPermission(AUTH_Edit_Customers)) {
				exit;
			}
		...
		private function ViewOrderNotes()
		{
			...
			// Load the order
			$order = GetOrder($_REQUEST['orderId']);
			if (!$order || ($this->auth->GetVendorId() && $order['ordvendorid'] != $this->auth->GetVendorId())) {
				exit;
			}
		...
		{
			if (!isset($_POST['type']) || $_POST['type'] == '' || !isset($_POST['discountId']) || !isId($_POST['discountId']) || !$this->auth->HasPermission(AUTH_Customer_Groups) || !gzte11(ISC_MEDIUMPRINT)) {
				$tags[] = $this->MakeXMLTag('status', 0);
