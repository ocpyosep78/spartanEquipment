	...
	class ISC_ADMIN_PAGES extends ISC_ADMIN_BASE
	{
		public function HandleToDo($Do)
		{
			...
			switch (isc_strtolower($Do))
			{
				case "editpage2":
				{
					if ($this->auth->HasPermission(AUTH_Edit_Pages)) {
				...
				case "createpage2":
				{
					if ($this->auth->HasPermission(AUTH_Add_Pages)) {
				...
				case "deletepages":
				{
					if ($this->auth->HasPermission(AUTH_Delete_Pages)) {
				...
				case "previewpage":
				{
					if ($this->auth->HasPermission(AUTH_Manage_Pages)) {
						$this->PreviewPage();
						break;
					} else {
		...
		private function ManagePages($MsgDesc = "", $MsgStatus = "")
		{
			...
			if(gzte11(ISC_HUGEPRINT) && $this->auth->GetVendorId() == 0) {
			...
			if (!$this->auth->HasPermission(AUTH_Delete_Pages) || !$GLOBALS['PageGrid']) {
		...
		private function EditVisibility()
		{
			...
			if($this->auth->GetVendorId() && $arrData['pagevendorid'] != $this->auth->GetVendorId()) {
		...
		private function deletePageSearch($pageId)
		{
			...
			if ($this->auth->GetVendorId()) {
				$query .= " AND pagevendorid=".(int)$this->auth->GetVendorId();
			}
		...
		private function AddPageStep1($MsgDesc = "", $MsgStatus = "", $IsError = false)
		{
			...
			if($IsError) {
				...
				$GLOBALS['SetupType'] = "SwitchType(0);";
				$GLOBALS['ParentPageOptions'] = $this->GetParentPageOptions(0, 0, $this->auth->GetVendorId());
		...
		public function GetParentPageOptions($SelectedPage = 0, $DontDisplay = 0, $vendorId=0)
		{
			...
			$pages = $nested->getTree(
				...
				null,
				true,
				array('pagevendorid = ' . $this->auth->GetVendorId()),
				$exclusions
			);
		...
		private function EditPageStep2()
		{
			...
			// Does this user have permission to edit this product?
			if($this->auth->GetVendorId() && $existingPage['pagevendorid'] != $this->auth->GetVendorId()) {
				FlashMessage(GetLang('Unauthorized'), MSG_ERROR, 'index.php?ToDo=viewPages');
			}
