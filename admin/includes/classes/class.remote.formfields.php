<?php
	...
	class ISC_ADMIN_REMOTE_FORMFIELDS extends ISC_ADMIN_REMOTE_BASE
	{
		...
		{
			if (!$this->auth->HasPermission(AUTH_Manage_FormFields)) {
		...
		{
			if (!$this->auth->HasPermission(AUTH_Edit_FormFields)) {
				exit;
			}
		...
		{
			if (!$this->auth->HasPermission(AUTH_Add_FormFields) || !gzte11(ISC_MEDIUMPRINT)) {
				exit;
			}
		...
		{
			if (!$this->auth->HasPermission(AUTH_Delete_FormFields) || !gzte11(ISC_MEDIUMPRINT)) {
				$tags[] = $this->MakeXMLTag('status', 0);
