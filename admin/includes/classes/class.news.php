	...
	class ISC_ADMIN_NEWS extends ISC_ADMIN_BASE
	{
		public function HandleToDo($Do)
		{
			...
			switch (isc_strtolower($Do))
			{
				case "prevnews":
				{
					if ($this->auth->HasPermission(AUTH_Manage_News)) {
						$this->PreviewNews();
					} else {
				...
				{
					if ($this->auth->HasPermission(AUTH_Edit_News)) {
				...
				case "addnews2":
				{
					if ($this->auth->HasPermission(AUTH_Add_News)) {
						$GLOBALS['BreadcrumEntries'] = array(GetLang('Home') => "index.php", GetLang('News') => "index.php?ToDo=viewNews");
				...
				case "deletenews":
				{
					if ($this->auth->HasPermission(AUTH_Delete_News)) {
						$GLOBALS['BreadcrumEntries'] = array(GetLang('Home') => "index.php", GetLang('News') => "index.php?ToDo=viewNews");
		...
		public function ManageNews($MsgDesc = "", $MsgStatus = "")
		{
			...
			if (!$this->auth->HasPermission(AUTH_Delete_News) || $numNews == 0) {
