...
class ISC_ADMIN_VENDORS extends ISC_ADMIN_BASE
{
	...
	public function HandleToDo($Do)
	{
		...
		if (!$this->auth->HasPermission(AUTH_Manage_Vendors)) {
			if($this->auth->GetVendorId() && isc_strtolower($Do) != 'editvendor' && isc_strtolower($Do) != 'saveupdatedvendor' && @$_REQUEST['vendorId'] != $this->auth->GetVendorId()) {
	...
	private function CommitVendor($data, $vendorId=0)
	{
		...
		if($this->auth->GetVendorId() == 0 || $vendorId == 0) {
	...
	private function SaveUpdatedVendor()
	{
		...
		if(!$this->CommitVendor($_POST, $vendor['vendorid'])) {
			...
			else if($this->auth->GetVendorId() || isset($_POST['addAnother'])) {
