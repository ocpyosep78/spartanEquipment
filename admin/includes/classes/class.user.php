...
class ISC_ADMIN_USER extends ISC_ADMIN_BASE
{
	...
	public function HandleToDo($Do)
	{
		...
		switch(isc_strtolower($Do)) {
			case "copyuser":
			{
				if($this->auth->HasPermission(AUTH_Add_User)) {
			...
			case "edituser2":
			{
				if($this->auth->HasPermission(AUTH_Edit_Users)) {
			...
			case "deleteusers":
			{
				if($this->auth->HasPermission(AUTH_Delete_Users)) {
					$GLOBALS['BreadcrumEntries'] = array(GetLang('Home') => "index.php", GetLang('Users') => "index.php?ToDo=viewUsers");
	...
	private function ManageUsersGrid(&$numUsers)
	{
		...
		if(gzte11(ISC_HUGEPRINT) && !$this->auth->GetVendorId()) {
		...
		if($numUsers > 0) {
			...
			while($row = $GLOBALS['ISC_CLASS_DB']->Fetch($userResult)) {
				...
				switch($row['userstatus'])
				{
					...
					case 1: // Active
					{
						if($this->auth->HasPermission(AUTH_Edit_Users) && !($row['pk_userid'] == 1) ) {
	...
	public function ManageUsers($MsgDesc = "", $MsgStatus = "", $PendingUser = false)
	{
		...
		if(!$this->auth->HasPermission(AUTH_Delete_Users) || $numUsers == 0) {
	...
	public function _GetUserList($Start, $SortField, $SortOrder, &$NumResults)
	{
		...
		if($this->auth->GetVendorId()) {
			$queryWhere .= " AND uservendorid='".(int)$this->auth->GetVendorId()."'";
		}
	...
	private function DeleteUsers()
	{
		if(isset($_POST['users'])) {
			...
			$vendorRestriction = '';
			if($this->auth->GetVendorId()) {
				$vendorRestriction = " AND uservendorid='".(int)$this->auth->GetVendorId()."'";
			}
	...
	private function AddUserStep1()
	{
		...
		if(!gzte11(ISC_HUGEPRINT)) {
			...
			if($this->auth->GetVendorId()) {
				$vendorDetails = $this->auth->GetVendor();
	...
	public function _CommitUser($UserId, &$Data, &$Perms, &$Err)
	{
		...
		if($UserId == 0) {
			...
			foreach($Perms as $p) {
				...
				if($this->auth->GetVendorId() > 0 && !in_array($p, $this->vendorOnlyPermissions) && !in_array($p, $this->vendorAdminPermissions)) {
					continue;
				}
				$newPermission = array(
					"permuserid" => $UserId,
	...
	private function _GetUserData($UserId, &$RefArray)
	{
		...
		if ($UserId == 0) {
			...
			if(gzte11(ISC_HUGEPRINT)) {
				if($this->auth->GetVendorId()) {
					$RefArray['uservendorid'] = $this->auth->GetVendorId();
				}
				else {
	...
	private function EditUserStep1()
	{
		...
		if(UserExists($userId)) {
			...
			if($this->auth->GetVendorId() && $arrData['uservendorid'] != $this->auth->GetVendorId()) {
	...
	private function EditUserStep2()
	{
		...
		// Does this user have permission to edit this user?
		if($this->auth->GetVendorId() && $arrUserData['uservendorid'] != $this->auth->GetVendorId()) {
			FlashMessage(GetLang('Unauthorized'), MSG_ERROR, 'index.php?ToDo=viewUsers');
		}
