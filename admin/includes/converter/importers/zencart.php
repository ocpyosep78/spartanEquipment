...
class ISC_ADMIN_CONVERTER_ZENCART extends ISC_ADMIN_CONVERTER
{
	...
	public function ImportOrders()
	{
		...
		while($order = $GLOBALS['ZENCART_DB']->Fetch($result)) {
			...
			$query2 = sprintf("
				SELECT op.*, p.products_weight, pd.products_attributes_filename, opa.products_options, opa.products_options_values
				...
				LEFT JOIN [|PREFIX|]orders_products_attributes opa ON (opa.orders_products_id = op.orders_products_id)
			...
			while($row = $GLOBALS['ZENCART_DB']->Fetch($result2)) {
				if($PrevOrdProdId != $row['orders_products_id']) {

					if($PrevOrdProdId!=0) {
						$prodOptions = serialize($OrdProdAttr);
						$OrdPro[$PrevOrdProdId]['ordprodoptions'] = $prodOptions;
					}
					...
					$OrdPro[$row['orders_products_id']] =  array(
					...
					$OrdProdAttr = array();
					$PrevOrdProdId = $row['orders_products_id'];
				}

				if ($row['products_options'] != '') {
					$OrdProdAttr[$row['products_options']] = $row['products_options_values'];
				}
			}

			if($PrevOrdProdId!=0 && isset($OrdPro[$PrevOrdProdId])) {
				$prodOptions = serialize($OrdProdAttr);
				$OrdPro[$PrevOrdProdId]['ordprodoptions'] = $prodOptions;
			}

			foreach($OrdPro as $OrdProdId => $row) {
				$importOrder['products'][] = array(
					...
					"orderprodqtyshipped" => 0,
					"ordprodid" => $row['products_id'],
					"ordprodoptionid" => 0,
					"ordprodtype" => $row['ordprodtype'],
					"ordprodoptions" => $row['ordprodoptions'],
				);
			}
			$err = '';
