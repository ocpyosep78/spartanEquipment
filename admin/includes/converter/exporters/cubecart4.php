...
class ISC_ADMIN_EXPORTER_CUBECART4 extends ISC_ADMIN_EXPORTER
{
	...
	public function InsertCategory($categoryid, $categoryData)
	{
	...
	public function ExportProductsCleanup()
	{
	...
	public function InsertReview($reviewid, $reviewData)
	{
		$review = array(
			"id" => $reviewid,
	...
	public function InsertProduct($productid, $productData)
	{
		$isDigital = 0;
		$digitalFile = '';
		if(is_array($productData['downloads'])) {
			...
			if($download && file_exists($download['downfile'])) {
				...
				if(!is_dir($this->_exportSession['Configuration']['path'] . "/downloads")) {
					@mkdir($this->_exportSession['Configuration']['path'] . "/downloads", ISC_WRITEABLE_DIR_PERM);
				}
		...
		if($firstImage) {
			...
			if(!is_dir($imageDir)) {
				@mkdir($imageDir, ISC_WRITEABLE_DIR_PERM);
			}
	...
	public function GetProductsOptionId($optname)
	{
		static $optcache;
	...
	public function GetProductsValueId($optvalue, $optid)
	{
		static $optcache;
	...
	public function InsertCustomer($customerid, $customerData)
	{
	...
	public function InsertOrder($orderId, $orderData)
	{
	...
	*/
	public function GetCubeCartISOCountry($country)
	{
		static $cache;
