...
class ISC_BATCH_IMPORTER_PRODUCTS extends ISC_BATCH_IMPORTER_BASE
{
	...
	private static $gpsCategoryCache = array();
	...
	public function __construct()
	{
		...
		$this->_ImportFields = array(
			...
			"gpsGlobalTradeItemNumber"=> GetLang("gpsGlobalTradeItemNumber"),
			"gpsManufacturerPartNumber"=> GetLang("gpsManufacturerPartNumber"),
			"gpsGender"=> GetLang("gpsGender"),
			"gpsAgeGroup"=> GetLang("gpsAgeGroup"),
			"gpsColor"=> GetLang("gpsColor"),
			"gpsSize"=> GetLang("gpsSize"),
			"gpsMaterial"=> GetLang("gpsMaterial"),
			"gpsPattern"=> GetLang("gpsPattern"),
			"gpsItemGroupId"=> GetLang("gpsItemGroupId"),
			"gpsCategory"=> GetLang("gpsCategory"),
			"gpsGooglePsEnabled"=> GetLang("gpsGooglePsEnabled"),
		);
	...
	protected function _ImportRecord($record)
	{
		...
		if(isset($record['brandname']) && $record['brandname'] != '') {
			...
			// Create new brand
				...
				if ($this->auth->HasPermission(AUTH_Add_Brands)) {
					$newBrand = array(
		...
		$productData = array(
			...
			"prodvisible" => (int)$this->StringToYesNoInt($record['prodvisible']),
			...
			'google_ps_enabled' => @$this->StringToYesNoInt($record['gpsGooglePsEnabled']),
		);
		// ISC-2949, ISC-387
		if ($this->ignoreBlankFields()) {
			// ensure we don't undo existing prodvisible value if none is provided in import data
			if ($record['prodvisible'] === '') {
				unset($productData["prodvisible"]);
			}

			// ISC-3556 Ensure we don't change the Google Product Search Enabled (Y/N)
			if (!isset($record['gpsGooglePsEnabled'])) {
				unset($productData['google_ps_enabled']);
			}

			// ISC-
			if(isset($record['prodinvtrack']) && empty($record['prodinvtrack'])) {
				unset($productData['prodinvtrack']);
			}

			// ensure we don't undo existing prodtags value if none is provided in import data
			if ($record['prodtags'] === '') {
				unset($productData['prodtags']);
			}
		}

		/**
		 * ISC-2949
		 * this should ensure that for new products we default the product to visible if
		 * blank value was supplied
		 */
		if (empty($existing) && $record['prodvisible'] === '') {
		...
		if($productId == 0) {
			$this->setCurrentProductId($GLOBALS['NewProductId']);
			$productId = $this->getCurrentProductId();
		}

		/**
		 * Handle the Google Product Search record.
		 */
		$this->handleGoogleProductSearch($record);
	...
	/**
	 * Handle any supplied Google product search information
	 * @param Array $record
	 */
	public function handleGoogleProductSearch($record)
	{
		// Initialise the results array.
		$result = array();

		/**
		 * Pass the record through a method which will remove elements that shouldnt be saved
		 */
		$processedRecord = $this->gpsProcessRecord($record);

		/**
		 * Now save the record.
		 */
		$gps = new ISC_ADMIN_GOOGLEPS_PRODUCT();
		$result = $gps->save($this->gpsMapRecordToSaver($processedRecord));

		if(!$result) {
			$this->addImportResult('Warnings', GetLang('gpsNoSave'));
			return false;
		}
	...
	public function gpsProcessRecord($record)
	{
		$processedRecord = $record;

		/**
		* ISC-3717 oversight
		* If you're adding products at the same time as mapping them to Google Product Search, it will fail without this line.
		*/
		if(!isset($processedRecord['productid']) || empty($processedRecord['productid'])) {
			$processedRecord['productid'] = $this->getCurrentProductId();
		}

		/**
		 * Validate the Age Groups.
		 */
		if(isset($processedRecord['gpsAgeGroup']) && !$this->gpsValidateAgeGroup($processedRecord['gpsAgeGroup'])) {
			$this->addImportResult('Warnings', GetLang('gpsAgeGroupInvalid', array('ageGroup' => $processedRecord['gpsAgeGroup'])));
			unset($processedRecord['gpsAgeGroup']);
		}

		/**
		 * Validate the Gender.
		 */
		if(isset($processedRecord['gpsGender']) && !$this->gpsValidateGender($processedRecord['gpsGender'])) {
			$this->addImportResult('Warnings', GetLang('gpsGenderInvalid', array('gender' => $processedRecord['gpsGender'])));
			unset($processedRecord['gpsGender']);
		}

		/**
		 * Validate the Category.
		 */
		if(isset($processedRecord['gpsCategory']) && !$this->gpsValidCategory($processedRecord['gpsCategory'], $processedRecord)) {
			$this->addImportResult('Warnings', GetLang('gpsCategoryNotValid', array('categoryString' => $processedRecord['gpsCategory'])));
			unset($processedRecord['gpsCategory']);
		}
	...
	//@todo Where are these defined, the same source should be used?
	public function gpsValidateAgeGroup($ageGroup)
	{
		if (empty ($ageGroup)) {
			return true;
		}
		$valid = array('kids', 'adult');
		return in_array(strtolower($ageGroup), $valid);
	}

	//@todo Where are these defined, the same source should be used?
	public function gpsValidateGender($gender)
	{
		if(empty($gender)) {
			return true;
		}
		$valid = array('male', 'female', 'unisex');
		return in_array(strtolower($gender), $valid);
	}


	/**
	 * Validation on the supplied category. Make sure its appropriate for the product.
	 * The $record is required as the categoryId element is added.
	 * @param String $category the category.
	 * @param Array $record the current row being processed.
	 */
	public function gpsValidCategory($category, &$record)
	{

		/**
		* Translate the category string into an ID
		*/
		$categoryIds = $this->categoryNamesToIds($category);
		...
		if(!empty($categoryIds)) {
			...
			/**
			 * Check that the category is one of the main products categories
			 */
			if(!$this->gpsValidCategoryForThisProduct($categoryId, (int) $this->getCurrentProductId())) {
				return false;

			} else {

				/**
				 * Assign the category into the main $record array as its value used/required in the save() method
				 */
				$record['gpsCategoryId'] = $categoryId;
				return true;
			}

		} else {
			$this->addImportResult('Warnings', GetLang('gpsCategoryNotFound', array('categoryString' => $category)));
			return false;
		}
	}

	public function gpsValidCategoryForThisProduct($categoryId, $productId)
	{
		$query = "select * from [|PREFIX|]categoryassociations where productid = '".$productId."' and categoryid = '".$categoryId."'";
		$result = $GLOBALS['ISC_CLASS_DB']->Query($query);
		$suitable = $GLOBALS['ISC_CLASS_DB']->Fetch($result);
		if($suitable) {
			return true;
		} else {
			return false;
	...
	/**
	 * Handle mappiong of incoming record to the format expected by the Data saver.
	 * @param Array $record
	 */
	protected function gpsMapRecordToSaver($record)
	{
		...
		$fieldMap = array(
			...
			array(
				'recordKey' => 'gpsCategoryId',
				'mappedKey' => 'categoryid',
				'function' => false,
			),
			array(
				'recordKey' => 'gpsGlobalTradeItemNumber',
				'mappedKey' => 'global_trade_item_number',
				'function' => false,
			),
			array(
				'recordKey' => 'gpsManufacturerPartNumber',
				'mappedKey' => 'manufacturer_part_number',
				'function' => false,
			),
			array(
				'recordKey' => 'gpsGender',
				'mappedKey' => 'gender',
				'function' => 'isc_strtolower',
			),
			array(
				'recordKey' => 'gpsAgeGroup',
				'mappedKey' => 'age_group',
				'function' => 'isc_strtolower',
			),
			array(
			...
			array(
				'recordKey' => 'gpsMaterial',
				'mappedKey' => 'material',
				'function' => false,
			),
			array(
				'recordKey' => 'gpsPattern',
				'mappedKey' => 'pattern',
				'function' => false,
			),
			array(
				'recordKey' => 'gpsItemGroupId',
				'mappedKey' => 'item_group_id',
				'function' => false,
			),
		...
		foreach($fieldMap as $mappingData) {

			if(isset($record[$mappingData['recordKey']])) {

				$function = $mappingData['function'];
				if($function) {
					$mapped[$mappingData['mappedKey']] = call_user_func($function, $record[$mappingData['recordKey']]);
				} else {
					$mapped[$mappingData['mappedKey']] = $record[$mappingData['recordKey']];
				}
	...
	/**
	 * Convert a / separated categiry name into an array of caetgoryIds
	 * @param String $categoryName
	 */
	public function categoryNamesToIds($categoryNameString)
	{

		// Return from cache
		if(!empty(self::$gpsCategoryCache[$categoryNameString])) {
			return self::$gpsCategoryCache[$categoryNameString];
		}

		$cats = array();
		$categories = explode("/", $categoryNameString);
		$parentId = 0;
		$lastCategoryId = 0;
		...
		foreach($categories as $category) {
			...
			if($category == '') {
				continue; // no more left so exit the foreach
			}
			...
			// Category doesnt exist. we have a problem.
			if(!$existingCategory) {
		...
		self::$gpsCategoryCache[$categoryNameString] = $cats;
		return $cats;
	...
	{
		if(empty($record['prodinvtrack']) && !$this->ignoreBlankFields()) {
			$record['prodinvtrack'] = 0;
			return $record;
		}

		switch(isc_strtolower($record['prodinvtrack'])) {
			case 'by product':
			case '1':
				$method = 1;
				break;
	...
	/**
	* Gets the id of the last processed product, or false if no product has been processed yet
	*
	* @return int
	*/
	public function getCurrentProductId()
	{
		if (empty($this->ImportSession['CurrentProductId'])) {
			return false;
		}

		return $this->ImportSession['CurrentProductId'];
	}

	/**
	* Sets the id of the last processed product.
	*
	* @param int $productId
	*/
	public function setCurrentProductId($productId)
	{
		$this->ImportSession['CurrentProductId'] = $productId;
	}
}
