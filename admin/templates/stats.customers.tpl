	...
	<tr>
		<td>
			...
			<div id="div0" style="padding-top:10px" class="text">
				<center>
					<strong><span style="display:{{ HideNoAdvancedStatsMessage|raw }}; color:#CACACA"><br />({% lang 'NoOrderData2Days' %})</span></strong>
				</center>
				<div id="flashcontent" style="width: 100%; clear: both;">
				</div>
				<script type="text/javascript" src="{{ ShopPath|raw }}/admin/includes/amcharts/swfobject.js?{{ JSCacheToken }}"></script>
				<script type="text/javascript">
					$(document).ready(function() {
						...
						so.addVariable("settings_file", escape("{{ ShopPath|raw }}/admin/includes/amcharts/customersbydate.xml"));
						so.addVariable("data_file", escape("{{ ShopPath|raw }}/admin/index.php?ToDo=custStatsByDateData&from={{ OverviewFromStamp|raw }}&to={{ OverviewToStamp|raw }}"));
	...
	function LoadCustomersByDateGrid() {
		...
		jQuery.ajax({url: 'index.php?ToDo=custStatsByDateGrid&FromLink='+customersByDateFromLink+'&From={{ FromStamp|raw }}&To={{ ToStamp|raw }}&Page='+customersByDateCurrentPage+'&Show='+customersPerPage+'&SortBy='+customersByDateSortField+'&SortOrder='+customersByDateSortOrder,
	...
	function LoadRevenuePerCustomerGrid() {
		...
		jQuery.ajax({url: 'index.php?ToDo=custStatsByRevenueGrid&FromLink='+revenuePerCustomerFromLink+'&From={{ FromStamp|raw }}&To={{ ToStamp|raw }}&Page='+revenuePerCustomerCurrentPage+'&Show='+customersPerPage+'&SortBy='+revenuePerCustomerSortField+'&SortOrder='+revenuePerCustomerSortOrder,
			     success: function(data) {
