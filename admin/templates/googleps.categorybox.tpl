		...
		<ul>
			...
			{% for category in categories %}
				...
				<li {% if selectedid == category.id %} class="SelectedRow" {% endif %}>
					<label>
						<input {% if selectedid == category.id %} checked="checked" {% endif %}type="radio" name="category_{{ category.parent_id }}"
							id="category_{{ category.id }}" value="{{ category.id }}" {% if category.num_children == 0 %}class="CategoryLeaf"{%else%}class="CategoryParentAndLeaf"{% endif %} />
