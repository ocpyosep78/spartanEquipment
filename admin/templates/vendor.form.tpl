<form action="index.php?ToDo={{ FormAction|raw }}" id="frmVendor" method="post" onsubmit="return ValidateForm(CheckVendorForm)" enctype="multipart/form-data">
	<input type="hidden" name="vendorId" id="vendorId" value="{{ VendorId|raw }}" />
		...
		<tr>
			<td class="Intro">
				...
				<p>
					...
					<input type="submit" name="SaveAddAnotherButton1" value="{{ SaveAndAddAnother|raw }}" name="addAnother" class="FormButton" style="width:130px" />
		...
		<tr>
			<td>
				<ul id="tabnav">
					...
					<li style="{{ HideShipping|raw }}"><a href="#" id="tab1" onclick="ShowTab(1)">{% lang 'VendorShipping' %}</a></li>
				</ul>
				<div id="div0">
					<table width="100%" class="Panel">
						...
						<tr>
							...
							<td>
								<input type="text" name="vendorname" id="vendorname" class="Field250" value="{{ VendorName|raw }}" />
						...
						<tr>
							...
							<td>
								<input type="text" name="vendorphone" id="vendorphone" class="Field250" value="{{ VendorPhone|raw }}" />
						...
						<tr>
							...
							<td class="PanelBottom">
								<input type="text" name="vendoremail" id="vendoremail" value="{{ VendorEmail|raw }}" class="Field250" />
						...
						<tr>
							...
							<td>
								<input type="text" name="vendoraddress" id="vendoraddress" value="{{ VendorAddress|raw }}" class="Field250" />
						...
						<tr>
							...
							<td>
								<input type="text" name="vendorcity" id="vendorcity" value="{{ VendorCity|raw }}" class="Field250" />
						...
						<tr>
							...
							<td class="Field">
								<select style="{{ HideStateList|raw }}" name="vendorstate" id="vendorstate" class="Field250">
									<option value="">{% lang 'ChooseState' %}</option>
									{{ StateList|raw }}
								</select>
								<input style="{{ HideStateBox|raw }}" type="text" name="vendorstate1" id="vendorstate1" class="Field250" value="{{ VendorState|raw }}" />
							</td>
						</tr>
						...
						<tr>
							...
							<td class="PanelBottom">
								<input type="text" name="vendorzip" id="vendorzip" value="{{ VendorZip|raw }}" class="Field50" />
						...
						<tr style="{{ HideLogoUpload|raw }}">
							...
							<td>
								...
								<span style="{{ HideCurrentVendorLogo|raw }}">
									<label><input name='deletevendorlogo' id='deletevendorlogo' type='checkbox' value='1' /> {% lang 'DeleteCurrentImage' %}</label> <a href="{{ CurrentVendorLogoLink|raw }}" target="_blank">{{ CurrentVendorLogo|raw }}</a>
								</span>
							</td>
						</tr>

						<tr style="{{ HidePhotoUpload|raw }}">
							<td class="FieldLabel">
								<span class="Required">&nbsp;</span>
							...
							<td>
								...
								<span style="{{ HideCurrentVendorPhoto|raw }}">
									<label><input name='deletevendorphoto' id='deletevendorphoto' type='checkbox' value='1' /> {% lang 'DeleteCurrentImage' %}</label> <a href="{{ CurrentVendorPhotoLink|raw }}" target="_blank">{{ CurrentVendorPhoto|raw }}</a>
								</span>
							</td>
						</tr>
						...
						<tr>
							...
							<td class="PanelBottom">
								<label> <input type="checkbox" name="forwardvendoremails" onclick="if(this.checked) { $('.ForwardInvoiceEmailsToggle').show(); } else { $('.ForwardInvoiceEmailsToggle').hide(); }" value="1" {{ VendorForwardInvoices|raw }} /> {% lang 'YesEnableForwardInvoiceEmails' %}</label>
								...
								<div style="margin-top: 3px; {{ HideForwardInvoiceEmails|raw }}" class="ForwardInvoiceEmailsToggle">
									<img src="images/nodejoin.gif" style="vertical-align: middle;" />
									<input type="text" name="vendororderemail" id="vendororderemail" class="Field250" value="{{ VendorOrderEmail|raw }}" /><br />
									<span class="Disabled" style='text-decoration: none; padding-left: 25px;'>{% lang 'ForwardOrderInvoicesDesc' %}</span>
								</div>
							</td>
						</tr>
						<tr style="{{ HidePermissions|raw }}" id="VendorProfitMarginFields">
							<td class="FieldLabel">
								<span class="Required">*</span> <label for="vendorprofitmargin">{% lang 'VendorProfitMargin' %}:</label>
							</td>
							<td class="PanelBottom">
								<input type="text" name="vendorprofitmargin" id="vendorprofitmargin" value="{{ VendorProfitMargin|raw }}" class="Field50" />
					...
					<table width="100%" class="Panel" style="{{ HidePermissions|raw }}">
						<tr>
						...
						<tr>
							...
							<td class="PanelBottom">
								<input type="checkbox" name="vendorlimitcats" id="vendorlimitcats" {{ AccessAllCategories|raw }} /> <label for="vendorlimitcats">{% lang 'VendorAccessAllAccess' %}</label>
								<span id="accesscatssel" style="{{ HideAccessCategories|raw }}">(<a href="#" id="selectAllCats">{% lang 'SelectAll' %}</a> / <a href="#" id="unselectAllCats">{% lang 'UnselectAll' %}</a>)</span>
								...
								<div style="padding-top:5px; {{ HideAccessCategories|raw }}" id="accesscategorylist">
				...
				<div id="div1" style="display: none">
					<table width="100%" class="Panel">
						...
						<tr>
							...
							<td>
								<label style="display: block;"><input type="radio" onclick="ToggleShipping(this.value)" name="vendorshipping" value="0" {{ VendorShippingDefault|raw }} /> {% lang 'VendorShippingStoreDefault' %}</label>
								<div id="StoreShippingMethods" style="{{ HideStoreMethodsList|raw }}">
									<img src="images/nodejoin.gif" alt="" class="FloatLeft" />
									{{ StoreShippingMethods|raw }}
								</div>

								<label style="display: block;"><input type="radio" onclick="ToggleShipping(this.value)" name="vendorshipping" value="1" {{ VendorShippingCustom|raw }} /> {% lang 'VendorShippingCustom' %}</label>
								<div id="ShippingZonesToggle" style="{{ HideShippingZonesGrid|raw }}">
									<div id="ShippingNotConfigured" style="{{ HideShippingNotConfigured|raw }}">
										<img src="images/nodejoin.gif" alt="" class="FloatLeft"/>
					...
					<div id="ShippingZonesGrid" style="{{ HideShippingZonesGrid|raw }}">
						<p style="padding-bottom: 0; margin-bottom: 10px; margin-top: 10px;">
						<input type="button" name="ZoneAddButton" value="{% lang 'AddShippingZoneButton' %}" class="SmallButton" onclick="document.location.href='index.php?ToDo=addShippingZone&vendorId={{ VendorId|raw }}';" />
							<input type="button" name="ZoneDeleteButton" value="{% lang 'DeleteSelected' %}" class="SmallButton" onclick="ConfirmDeleteSelectedZones();" {{ DisableDeleteZones|raw }} />
						</p>
						{{ NoZonesMessage|raw }}
						<div class="GridContainer" style="{{ DisplayZoneGrid|raw }}">
							{{ ShippingZonesGrid|raw }}
						</div>
					</div>
				</div>
				<table border="0" cellspacing="0" cellpadding="2" width="100%" class="PanelPlain">
					<tr>
						<td>
							...
							<input type="submit" name="SaveAddAnotherButton2" value="{{ SaveAndAddAnother|raw }}" name="addAnother" class="FormButton" style="width:130px" />
