...
<input type="hidden" name="product_ids" value="{{ ProductIds|raw }}" />
...
<script type="text/javascript">//<![CDATA[
	...
	function ExpandCategories(ProductId, Img) {
		...
		$('#category_'+ProductId).css('height', '50px');
	...
	$(document).ready(function() {
		{% for productId, categoryData in productCategoryData %}
			{{ category.categoryPicker("category_" ~ productId, categoryData.categories, true, categoryData.selectedCategories) }}
		{% endfor %}
	});
